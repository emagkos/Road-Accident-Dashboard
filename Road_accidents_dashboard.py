# lisbon_accidents_dashboard.py
# Capstone – Lisbon Road Accident Dashboard
...
def load_data(path):
    ...
    # If the street column exists but is effectively empty (all blank/NaN), disable it
    if street_col is not None:
        _non_empty = (gdf[street_col].astype(str).str.strip().replace({"": np.nan}).dropna())
        if _non_empty.empty:
...
with col4:
    if STREET_COL is not None and not df.empty:
        topn = (df[STREET_COL].astype(str).str.strip().replace({"": np.nan}).dropna()
